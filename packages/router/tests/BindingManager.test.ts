...
describe('BindingManager', () => {
  ...
  describe('getActiveDevice', () => {
    ...
    // Bug reproduction: activeDeviceId points to a device not in the devices array
    // This can happen when bindings.json is corrupted or migrated incorrectly
    it('should return null when activeDeviceId points to a non-existent device', async () => {
      const openId = 'ou_test_user';
      await bindingManager.bindUser(openId, 'dev_1', 'Device-1');

      // Manually corrupt the binding: activeDeviceId points to a ghost device
      const binding = await bindingManager.getUserBinding(openId);
      binding!.activeDeviceId = 'dev_ghost_not_in_devices';
      await store.setUserBinding(openId, binding!);

      // getActiveDevice should return null, not crash
      const activeDevice = await bindingManager.getActiveDevice(openId);
      expect(activeDevice).toBeNull();
    });
  });

  describe('new machine rebind scenario', () => {
    // Bug reproduction: user has existing binding from old machine (dev_old),
    // then runs `remote-cli init --force` on a NEW machine (dev_new) and binds again.
    // Expected: new device should become active (or at least getActiveDevice should not
    // return null just because the old machine's deviceId is still the activeDeviceId).
    it('should make new device active when old activeDeviceId no longer exists in devices', async () => {
      const openId = 'ou_test_user';

      // Step 1: user binds old machine
      await bindingManager.bindUser(openId, 'dev_old_machine', 'Old Machine');

      // Step 2: simulate old device being removed (e.g., manual cleanup or data drift)
      // but activeDeviceId still points to it — corrupt state
      const binding = await bindingManager.getUserBinding(openId);
      // Remove the old device from devices[] but leave activeDeviceId pointing at it
      binding!.devices = [];
      binding!.activeDeviceId = 'dev_old_machine';
      await store.setUserBinding(openId, binding!);

      // Step 3: user binds new machine
      await bindingManager.bindUser(openId, 'dev_new_machine', 'New Machine');

      // getActiveDevice should NOT return null — user should be able to execute commands
      const activeDevice = await bindingManager.getActiveDevice(openId);
      expect(activeDevice).not.toBeNull();
      expect(activeDevice?.deviceId).toBe('dev_new_machine');
    });

    it('should set second device as active when first device is gone and user rebinds', async () => {
      const openId = 'ou_test_user';

      // Step 1: user has two devices, dev_1 is active
      await bindingManager.bindUser(openId, 'dev_1', 'Device-1');
      await bindingManager.bindUser(openId, 'dev_2', 'Device-2');

      // Step 2: dev_1 is unbound (active device removed), dev_2 is promoted
      await bindingManager.unbindDevice(openId, 'dev_1');

      // Step 3: user now runs init --force on dev_1's machine again and rebinds
      await bindingManager.bindUser(openId, 'dev_1', 'Device-1-Reinstalled');

      // dev_2 should still be active (re-adding dev_1 should not hijack active status)
      // AND getActiveDevice must return a valid device (not null)
      const activeDevice = await bindingManager.getActiveDevice(openId);
      expect(activeDevice).not.toBeNull();
    });
  });
