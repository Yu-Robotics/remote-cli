...
describe('MessageHandler', () => {
  ...
  describe('/thread commands', () => {
    it('should return error when threadManager is not set', async () => {
      // handler was created without threadManager (default testsetup)
      const message = {
        type: 'command',
      ...
      await handler.handleMessage(message);
      expect(mockWsClient.send).toHaveBeenCalledWith(
        expect.objectContaining({ success: false, error: expect.stringContaining('not available') })
      );
    });

    it('should list threads via /thread when threadManager is provided', async () => {
      const mockThreadManager = {
        listThreads: vi.fn(() => [
          { id: 'id-1', name: 'main', sessionId: null, workingDirectory: '/tmp', createdAt: 1, lastActiveAt: 1 },
        ]),
        getActiveThread: vi.fn(() => ({ id: 'id-1', name: 'main', sessionId: null, workingDirectory: '/tmp', createdAt: 1, lastActiveAt: 1 })),
        getThreadSummaries: vi.fn(() => [{ id: 'id-1', name: 'main', isActive: true }]),
        enableExternalSessionManagement: vi.fn(),
      } as any;

      const threadHandler = new MessageHandler(
        mockWsClient,
        mockExecutor,
        directoryGuard,
        mockConfig,
      ...
      await threadHandler.handleMessage({
        type: 'command',
        messageId: 'msg-list',
        content: '/thread list',
        timestamp: Date.now(),
      ...
      expect(mockWsClient.send).toHaveBeenCalledWith(
        expect.objectContaining({ success: true, output: expect.stringContaining('main') })
      );
    });

    it('should include thread summaries in response when threadManager is set', async () => {
      const summaries = [{ id: 'id-1', name: 'main', isActive: true }];
      const mockThreadManager = {
        listThreads: vi.fn(() => []),
        getActiveThread: vi.fn(() => undefined),
        getThreadSummaries: vi.fn(() => summaries),
        enableExternalSessionManagement: vi.fn(),
      } as any;
      ...
      await threadHandler.handleMessage({
        type: 'command',
        messageId: 'msg-with-threads',
        content: 'hello',
        timestamp: Date.now(),
      });

      expect(mockWsClient.send).toHaveBeenCalledWith(
        expect.objectContaining({ threads: summaries })
      );
    });
  });
});
