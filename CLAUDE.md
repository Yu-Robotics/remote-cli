...
- **Resume**: the `conversation_id` is persisted per thread (`~/.remote-cli/agy-sessions/<threadId>.json`) and passed via `--conversation <id>` when respawning (process crash, `setWorkingDirectory`, abort). A spawn with no stored id passes `--new-project` so the thread starts in a dedicated agy project — a bare spawn would join agy's shared `default-cli-project`, where restart auto-resume can attach the fresh process to another thread's conversation (cross-thread memory bleed).
