...
- **Per-thread data isolation**: agy stores all conversations under `$HOME/.gemini/antigravity-cli` — one global store shared by every thread, so a fresh agent could read other threads' transcripts when asked to recall past chats (cross-thread memory bleed). The executor therefore spawns agy with `HOME=~/.remote-cli/agy-homes/<threadId>`: auth/config files (`oauth_creds.json`, `settings.json`, ...) and program directories (`bin`, `builtin`, `cache`, `updater`) are symlinked from the real `~/.gemini`, while `brain/` and `conversations/` are per-thread. Existing conversations require a one-time manual migration before upgrading; startup never copies conversation data. If agy rewrites a symlinked credential file (token refresh), the local copy is synced back to the master and re-linked on the next spawn. Setup failure stops AGY rather than using the shared HOME. `/thread delete` removes the thread HOME when its AGY executor is active.
