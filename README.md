...
When the Router restarts or its WebSocket connection drops, running backend processes continue. With CLI and Router 1.6.52 or newer, the CLI automatically reconnects and creates a new recovery card for each running task; old cards remain unchanged. If the old card's session survived on the Router (a reconnect quick enough to beat disconnect cleanup), the Router instead adopts that card: retained output stays visible and new output continues in the same card after a separator marking the possible gap. The recovery card marks the output gap and displays only output produced after that card is ready. Output during disconnection or recovery is discarded, not buffered or replayed. The first resumed text segment uses plain text until a tool or image boundary to avoid broken Markdown fragments. Tasks that finish offline report only their completion or failure status. The CLI retains at most 100 such terminal records for up to 24 hours, in memory; restarting the CLI discards them. Recovery cards are requested one at a time, retrying with exponential backoff and giving up after a bounded number of attempts. A graceful Router stop finalizes in-flight task cards as interrupted. Automatic updates wait for pending task results to be acknowledged (or expire). Older Router versions keep the previous reconnect behavior.
