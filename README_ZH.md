...
Claude 沙箱只对 Bash 系命令做 OS 级强制。在 workspace-write 模式下，进程级的 `PreToolUse` hook 会自动放行当前工作区和已授权可写目录内普通的 `Write`、`Edit`、`NotebookEdit` 操作，并检查实际目标（包括符号链接和新文件的既有父目录）。目录之外的文件、含混路径、硬链接以及受保护的配置/凭据文件仍需审批。原生 deny 和 ask 规则即使在已授权目录内也保持优先。其他工具保持原生权限流程。沙箱内命令不弹确认直接运行；需要扩大权限的请求在支持时使用权限请求卡片。旧版 Router 或卡片创建失败时使用文字回复：单个待决请求回 `yes` 或 `no`，多个待决请求回 `yes <请求 ID>` / `no <请求 ID>`。目录之外的文件审批会提供 **Allow and remember directory**（允许并记住目录）：记住会批准这一次、把该目录的原生 `Write`/`Edit` 允许规则写入项目本地设置、在当前会话立即生效，并把该目录加入此线程保存的可写根目录，供未来 Bash 沙箱使用。命令和权限类审批只生效一次，不提供记住。如果卡片和文字输出都不可用，请求会被拒绝。任务完成、失败、超时、中止、上下文重置和进程终止都会使待决审批失效。Claude Code 还会让可写目录内它自己的设置和 hook 文件保持只读。
