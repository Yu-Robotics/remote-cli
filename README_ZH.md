...
当 Router 重启或 WebSocket 连接断开时，正在运行的 backend 进程会继续执行。CLI 和 Router 均为 1.6.52 或更高版本时，CLI 会自动重连并为每个运行中的任务创建新的恢复卡片；旧卡片保持不变。如果旧卡片的会话在 Router 上仍然存活（重连足够快，赶在断连清理之前），Router 会直接收养该卡片：已保留的输出继续可见，新输出在标记可能缺口的分隔符之后继续写入同一张卡片。恢复卡片会标记输出缺口，并且只显示该卡片就绪之后产生的输出。断连或恢复期间的输出会被丢弃，不会缓存或重放。恢复后的第一个文本片段会使用纯文本，直到遇到工具或图片边界，以避免 Markdown 片段破损。离线期间结束的任务只报告其完成或失败状态。CLI 最多在内存中保留 100 条此类终止记录，最长 24 小时；重启 CLI 会丢弃它们。恢复卡片一次只请求一张，失败时按指数退避重试，并在有限次数后放弃。Router 优雅停止时会把进行中的任务卡片 finalize 为已中断。自动更新会等待未决的任务结果得到确认（或过期）。旧版本 Router 保持之前的重连行为。
